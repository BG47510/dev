...
extract_and_filter() {
    ...
    # Variable pour indiquer si on a déjà trouvé un programme dans un channel
    ...
    while IFS= read -r line; do
        ...
        for channel_id in "${CHANNEL_IDS[@]}"; do
            ...
            # Si on a trouvé le channel précédemment, lire et ajouter les programmes
            if [ "$found_channel" = true ]; then
                # Lire les programmes jusqu'à ce que l'on trouve une nouvelle chaîne ou fin de fichier
                while IFS= read -r programme_line; do
                    echo "$programme_line" >> $OUTPUT_FILE
                    if [[ "$programme_line" == *"</programme>"* ]]; then
                        break
                    fi
                    # Vérifier si on saute à une nouvelle chaîne
                    if echo "$programme_line" | grep -q "<channel"; then
                        break
                    fi
                done < <(grep -A 10 -m 1 "<programme channel=\"$channel_id\">" "$tmp_file")
                found_channel=false  # Resetter la variable pour le prochain channel
            fi
        done
    done < "$tmp_file"
