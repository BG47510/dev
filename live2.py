...
class TeleramaXMLTVGrabber:
    """Met en œuvre les fonctionnalités de saisie et de traitement nécessaires à la génération de données XMLTV à partir de
    l'api mobile Télérama.
    """
...
##############################################################################################################################################
    ...
    def _fix_xml_unicode_string(self, text):
        """Remplace dans une chaîne tous les caractères spécifiques à Windows-1252 en UTF-8 et supprime les
        caractères non compatibles XML.
        """
        fixed_text = ''.join(
...
#########################################################################################################################################################
    ...
    def _parse_program_dict(self, program):
        ...
        if program['serie'] and program['serie']['numero_episode']:
            program_dict['season'] = (
                program['serie'].get('saison', 1) or 1) - 1
        ...
        if program['note_telerama'] > 0:
            # Pour avoir sur 10 pour Kodi
            program_dict['rating'] = float(program['note_telerama'] * 2)

        return program_dict
#######################################################################################################################################################

    def _query_telerama_api(self, procedure, **query):
        """Interroge l’API Télérama."""
...
###############################################################################################################################################
    ...
    def _get_current_programs(self, channels):
        """Retrouvez les programmes actuels de Télérama pour une chaîne donnée et un jour donné."""
...
def grab_tv_guide(channels):
    telerama = TeleramaXMLTVGrabber()  # Déclenche la classe
